...
impl AudioEngine {
    ...
    pub fn play_processed(
        ...
        let file_duration_secs =
            cached.samples.len() as f32 / channels.max(1) as f32 / sample_rate.max(1) as f32;
        let original_trimmed_duration_secs = sound.trimmed_length().max(0.05);
        let original_offset_secs =
            (start_position_secs - sound.trim_start_secs).clamp(0.0, original_trimmed_duration_secs);
        let file_offset_secs = (original_offset_secs / speed).clamp(0.0, file_duration_secs.max(0.0));
        ...
        self.current_total_duration_secs = original_trimmed_duration_secs;
        self.current_trim_start_secs = sound.trim_start_secs;
        self.current_start_offset_secs = original_offset_secs.min(original_trimmed_duration_secs);
        self.current_speed = speed;
        self.sink = Some(sink);
        Ok(())
    }
